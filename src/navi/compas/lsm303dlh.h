...
static bool magInit() {
    ...
    // full scale
    // Наткнулся на проблему. При чувствительности 1.3 значения по всем осям
    // меняются примерно в диапазоне -700..+700. Но на некоторых экземплярах
    // по некоторым осям (у меня было на Y на двух экземплярах) при выходе
    // за диапазон -250..+250 возвращает переполнение (-4096),
    // при этом остальные оси спокойно показывают значения всё в том же
    // диапазоне -700..+700.
    // При уменьшении чувствительности до 1.9 все оси показывают примерно -300..+300,
    // но переполнения уже нет.
    // Возможно, это проблема после пайки, т.к. самый первый экземпляр из той же партии
    // работает нормально. После перепайки на новый экземпляр из той же партии -
    // проблема ушла.
