...
for f in simple/*.md; do cat "$f"; echo; echo "\\pagebreak"; done | pandoc -f markdown -o xdeya-simple.pdf --pdf-engine=xelatex --toc -N -M date="`date "+%e %B, %Y"`" --resource-path=simple --lua-filter ../fix-links-single-file.lua
