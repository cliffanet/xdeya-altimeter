...
1. Момент отделения. Вот так выглядит нестабильность показаний высотомера в условиях сильных турбулентных потоко у обреза двери.

2. Скачок замеров в момент атоматического старта записи трека. Здесь всё устройство подвисает, пока пишетпоказания за предыдущее время на протяжении около 5 секунд.
...
`Сибестоимость устройства:` `v.0.3` = 2500р, `v.0.4` = 3000р - это только детали и корпус, без учёта сборки и обслуживания, т.к. делаю сам. Стоимость продолжает увеличиваться при нынешней инфляции и росте курса доллара.
